...
const AddPage = () => {
  ...
  return (
    ...
    <div onClick={() => addMeal()} className='min-w-250 my-5 h-80 w-[31%] place-self-center justify-center text-center flex flex-col border-teal border-dotted rounded-lg border-2'>
    <p className='w-full text-4xl font-bold text-center text-teal'>+</p>
    <p className='w-full text-xl text-center mt-5 text-teal'>add a new meal</p>
        </div>
        {
            meals?.length>0 && meals?.map((meal) => (
                <div key={meal._id} className={`${meal.available? 'opacity-100': 'opacity-50'} min-w-250 my-5 h-full w-[31%] place-self-center center text-center flex flex-col border-[#393C49] rounded-lg border-2`}>
                    <img className='rounded-lg w-full h-36 object-cover' src={meal.img} alt={`${meal.name}-img`}/>
                     <p className='text-md font-bold text-white place-self-center w-full text-center my-3'>{meal.name}</p>
