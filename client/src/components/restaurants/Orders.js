...
const Orders = () => {
    ...
    const levelUp = async(order) => {
        ...
        if (order.completed!=='completed'){
            try{
                await axios.put('http://localhost:5000/levelUp', {id:order._id, completed:level})
            }catch(err){
                console.log(err)
            }
        }
    ...
    <div className='p-20 bg-base h-full min-h-screen w-full text-white'>
         ...
         <div className='w-full space-between  mt-10 flex-row flex-wrap hidden lg:flex'>
                <div className='w-3/12 mr-10 p-3 min-h-100 bg-dblue rounded-lg h-full'>
                    ...
                    <FontAwesomeIcon className='w-full h-full' color='lightGreen' icon={faPlateWheat}/>
                    </div>
                ...
                <div className='w-3/12 mr-10 p-3 min-h-100 bg-dblue rounded-lg h-full'>
                    <div className=' p-3 bg-base rounded-lg w-12 h-12'>
                    <FontAwesomeIcon className='w-full h-full' color='pink' icon={faCircleDollarToSlot}/>
                    </div>
                ...
                <div className='w-3/12 p-3 min-h-100 bg-dblue rounded-lg h-full'>
                    <div className=' p-3 bg-base rounded-lg w-12 h-12'>
                    <FontAwesomeIcon className='w-full h-full' color='lightBlue' icon={faPerson}/>
                    </div>
         ...
         <div className='w-full p-12 mt-10 overflow-auto  bg-dblue'>
            ...
            <p className='min-w-250 w-250 text-lg mt-2 text-white italic font-bold'>Customer</p>
            ...
            <div className='mt-5 max-h-80'>
            {
                theOrders?.map(order => (
                    ...
                    <div onClick={() => levelUp(order)} className={`${order.completed === 'pending' && 'bg-red-400'} ${order.completed === 'created' && 'bg-purple-400'} ${order.completed === 'completed' && 'bg-teal'}  mt-2 hover:scale-x-105 cursor-pointer w-200 min-w-200 place-content-center flex flex-row h-10 rounded-lg  px-3 py-2`}>
                     <p className='text-md place-self-center text-white'>{order.completed}</p>
                     </div>
